...
def main():
    ...
    # Adriano Code
    st.title("AgroAssistent App")

    # Adriano Code
    image_url = st.text_input("Image URL", "https://bugwoodcloud.org/images/1536x1024/1453261.jpg")
    if st.button("Classify Image"):
        classification = api_client.classify_image(image_url)
        if classification:
            st.image(image_url)
            st.write("Classification:", classification)
    
    st.header("Registered Products")
    pest_name = st.text_input("Enter pest name:")

    # Adriano Code
    if st.button("Get Registered Products"):
        if pest_name:
            try:
                products = api_client.get_registered_products(pest_name)
                if products:
                    st.dataframe(products, height=400, width=800)
                else:
                    st.write("No products found for the given pest.")
    ...
    if st.session_state.user_input != "":
        ...
        llm_response = api_client.get_questions(user_input)
        chat_history.add_ai_message(llm_response["result"]+llm_response["observation"])
    ...
    #                 st.session_state.user_question = ""
    #             llm_answer = handle_image(uploaded_image)
    #             chat_history.add_user_message(user_message)
    #             chat_history.add_ai_message(llm_answer["pestClassification"]+'\n'+llm_answer["result"])
